...
// To run this need to have espeak-ng installed. I had issues with header
// linking so using explicit path in import.
// gcc -I/usr/local/Cellar/espeak-ng/1.51/include -o speech_test testing.c
// -L/usr/local/Cellar/espeak-ng/1.51/lib -lespeak-ng
//./speech_test

// Getting a warning here about pragma but it seems to be wrong
#pragma pack(push, 1)
typedef struct {
  char chunkID[4];        // "RIFF"
...
int main() {
  ...
  espeak_ERROR error = espeak_Synth(
      text,             // input text
      strlen(text) + 1, // input text length
      0,                // start position in input text
      POS_CHARACTER,    // defines if start position is words/chars etc
      0,                // end position
      espeakCHARS_AUTO | espeakPHONEMES | espeakENDPAUSE, // speach flags
      NULL,                                               // response unique ID
      NULL // user data, arbitary data to return with callbacks
  );
